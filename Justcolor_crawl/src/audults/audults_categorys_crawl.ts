...
const audults_categorys_crawl = async () => {
  ...
  const audults_categorys_titles_original =
    audults_dom.window.document.querySelector(".inside-left-sidebar").querySelectorAll("dt.level1.parent");;
  // handling audults_categorys_titles_original query to [{category_title:string}]
  const audults_categorys_images_handling = [] as any[];
  ...
  for (let i = 0; i < audults_categorys_titles_original.length; i++) {
    ...
    audults_categorys_images_handling.push({
      category_title: (audults_categorys_titles_original[i]?.querySelector('span')?.textContent as string).replaceAll("\n", ""),
      galleries: [],
    });
  }
