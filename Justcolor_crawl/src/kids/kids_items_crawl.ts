...
//   });
// };
const kids_item_detail_crawl = async (item_link: string) => {
  try {
    ...
    return {
      ...
      item_link,
      item_image: (kids_item_dom?.querySelector("#zoomcolor")?.querySelector("#zoomcolorimg") as HTMLImageElement)?.src,
...
const kids_items_crawl = async () => {
  ...
  for (const category of Array.from(kids_categorys_data)) {
    ...
    for (const kids_gallery_data of category.galleries) {
      ...
      const kids_gallery_dom =
        kids_dom.window.document.querySelectorAll(".entry-content");
      for (const kids_item_dom of kids_gallery_dom) {
        ...
        const kids_item_navigation_handling =
          kids_item_dom?.querySelectorAll(".page-numbers")[
            kids_item_dom?.querySelectorAll(".page-numbers")?.length - 1
          ]?.textContent || 1;
        ...
        for (let navigation = 1; navigation <= kids_item_navigation_handling; navigation++) {
          ...
          const kids_items_dom =
            items_dom.window.document.querySelectorAll(".grid-item");
          for (const kids_item_dom of kids_items_dom as Array<HTMLElement>) {
            //**=======================================**
            //**=======================================**
            // {
            //   item_title: "string",
