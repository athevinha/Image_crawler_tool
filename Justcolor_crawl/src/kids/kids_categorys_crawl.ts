...
const kids_categorys_crawl = async () => {
  ...
  const kids_categorys_titles_original =
    kids_dom.window.document.querySelector(".inside-left-sidebar").querySelectorAll("dt.level1.parent");;
  // handling kids_categorys_titles_original query to [{category_title:string}]
  const kids_categorys_images_handling = [] as any[];
  ...
  for (let i = 0; i < kids_categorys_titles_original.length; i++) {
    ...
    kids_categorys_images_handling.push({
      category_title: (kids_categorys_titles_original[i]?.querySelector('span')?.textContent as string).replaceAll("\n", ""),
      galleries: [],
    });
  }
