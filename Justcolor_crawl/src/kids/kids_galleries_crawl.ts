...
const kids_galleries_crawl = async () => {
  ...
  // query category dom
  const kids_category_dom =
    kids_dom.window.document.querySelector(".inside-left-sidebar").querySelectorAll("dd.level1.parent");
  ...
  for (let i = 0; i < kids_categorys_data.length; i++) {
    const kids_galleries_dom = (kids_category_dom[i] as HTMLElement)?.querySelectorAll("dt.level2")
    Array.from(kids_galleries_dom).forEach((kids_gallery_dom: HTMLElement) => {
      ...
      if (kids_gallery_dom.className.includes("notparent"))
        kids_categorys_data[i].galleries.push({
          gallery_title:
            kids_gallery_dom.querySelector("a").textContent,
        ...
        Array.from(kids_gallery_parents_dom).forEach((kids_gallery_parent_dom: Element) => {
          // console.log(kids_gallery_parent_dom)
          kids_categorys_data[i].galleries.push({
            gallery_title:
              kids_gallery_parent_dom.querySelector("a").textContent,
