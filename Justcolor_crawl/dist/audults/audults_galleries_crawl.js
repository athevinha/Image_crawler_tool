...
const audults_galleries_crawl = async () => {
    ...
    // query category dom
    const audults_category_dom = audults_dom.window.document.querySelector(".inside-left-sidebar").querySelectorAll("dd.level1.parent");
    for (let i = 0; i < audults_categorys_data.length; i++) {
        const audults_galleries_dom = audults_category_dom[i].querySelectorAll("dt.level2");
        Array.from(audults_galleries_dom).forEach((audults_gallery_dom) => {
            ...
            if (audults_gallery_dom.className.includes("notparent"))
                audults_categorys_data[i].galleries.push({
                    gallery_title: audults_gallery_dom.querySelector("a").textContent,
                ...
                Array.from(audults_gallery_parents_dom).forEach((audults_gallery_parent_dom) => {
                    // console.log(audults_gallery_parent_dom)
                    audults_categorys_data[i].galleries.push({
                        gallery_title: audults_gallery_parent_dom.querySelector("a").textContent,
...
//# sourceMappingURL=audults_galleries_crawl.js.map