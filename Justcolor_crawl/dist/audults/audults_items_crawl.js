...
const audults_item_detail_crawl = async (item_link) => {
    try {
        ...
        const audults_item_dom = item_dom.window.document.querySelector(".inside-article");
        return {
            item_title: audults_item_dom?.querySelector(".h1_bread")?.textContent?.replace("Coloring page : ", ""),
            item_link,
            item_image: audults_item_dom?.querySelector("#zoomcolor")?.querySelector("#zoomcolorimg")?.src,
...
const audults_items_crawl = async () => {
    ...
    const audults_gallery_dom = audults_dom.window.document.querySelectorAll(".entry-content");
    ...
    const audults_item_navigation_handling = audults_item_dom?.querySelectorAll(".page-numbers")[audults_item_dom?.querySelectorAll(".page-numbers")?.length - 1]?.textContent || 1;
    for (let navigation = 1; navigation <= audults_item_navigation_handling; navigation++) {
        ...
        const audults_items_dom = items_dom.window.document.querySelectorAll(".grid-item");
        for (const audults_item_dom of audults_items_dom) {
            //**=======================================**
            //**=======================================**
            // {
            //   item_title: "string",
...
//# sourceMappingURL=audults_items_crawl.js.map